...
# Build a final int model, removing non significant at $alpha = 0.1$
model.fin <- glm(V4399 ~ V3013 + V3015 + V3017 + V3048 + V3054 + V3083 + V3077 + PERINTVNUM + V3034:V3044 + V3034:V3048 + V3034:V3081 + V3048:V3081, data= crime.red2, family = "binomial")
model.fin <- update(model.fin, . ~ . - V3034No:V3044Yes)
model.fin <- update(model.fin, . ~ .  - V3034Yes:V3044No)
summary(model.fin)

model.fin <- glm(formula = V4399 ~ V3013 + V3015 + V3017 + V3048 + V3054 + 
...
model.gam <- gam(V4399 ~ s(V3013,3) + V3015 + V3017 + V3048 + V3054 + V3083 + V3077 + PERINTVNUM + V3034:V3044 + V3034:V3048 + V3034:V3081 + V3048:V3081, family = "binomial", data = crime.red2)
summary(model.gam)
```
...
res.P.int = residuals(model.step.int, type="pearson")
res.D.int = residuals(model.step.int, type="deviance") #or residuals(fit), by default
boxplot(cbind(res.P.int, res.D.int), names = c("Pearson", "Deviance"))

ggplotly(
  ggplot() + aes(x = model.step.int$fitted.values, y = res.D.int) + geom_point() + stat_spline(spar = 1.2, size = 0.8, color = "firebrick") + labs(x = "Fitted Values", y = "Deviance Residuals", title = "Deviances Residuals vs Fitted Values for Model with Interactions")
)
