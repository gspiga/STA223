...
NAprop <- crime.join.per %>% dplyr::summarise(across(everything(), ~ (sum(is.na(.)) / nrow(crime.join.per))))
...
length(sapply(1:ncol(crime.red), function(i) length(table(crime.red[, i]))) > 3)
...
model.red.int <-
  glm(
    V4399 ~ (
     V3013 + V3015 + V3044 + V3048 + V3054 + 
    V3077 + V3081 + PERINTVNUM + V4529
    ) ^ 2,
    data = crime.red2,
    family = "binomial"
  )
...
model.fin <-
  glm(V4399 ~ 
    V3013 + V3015 + V3044 + V3048 + V3054 + 
    V3077 + V3081 + PERINTVNUM + V4529 + V3044:V3077 + V3054:V3081,
    data = crime.red2,
    family = "binomial"
  ) # Model excludes V3034 and the interaction term between 34 and 44 is now a new variable encoded, int term between 34 and 48 also encoded
#model.fin <- update(model.fin, . ~ . - V3034No:V3044Yes)
#model.fin <- update(model.fin, . ~ .  - V3034Yes:V3044No)
summary(model.fin)

# Experimenting with gam
model.gam <- gam(V4399 ~ s(V3013,3) + V3015 + V3044 + V3048 + V3054 + 
    V3077  + PERINTVNUM + V4529 + V3044:V3077 + V3054:V3081, family = "binomial", data = crime.red2)
summary(model.gam)
...
# Our final model
$$
logit(E(Y|X)) = \beta_0 + \beta_1X_{Age} + \beta_2X_{MaritalStatus} + \beta_3X_{OffenderKnown} + \beta_4X_{ReportOther} + \beta_5
