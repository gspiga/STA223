...
ID.report <- da38090.0004[,c(2,3,4, 455)]
#ID.report

labels.4 <- attr(da38090.0004, "variable.labels")
labels.ls4 <- strsplit(labels.4, split = "'")[c(2, 3,4,455)]
...
cols2drop <- c("IDHH", "IDPER", "V3001", "V3002", "V3003", "V3004", "V3005", "V3008", "V3009", "V3010", "V3014","V3016", "V3018", "V3024A", "V3026", "V3027","V3060", "V3069", "V3082", "V3085", "V3086", "WGTPERCY", "PER_TIS", "PERBOUNDED")

# Count the NAs in all columns and find what is the proportion of them to the entire data set
NAprop <- crime.join %>% dplyr::select(everything()) %>% summarise_all(funs(sum(is.na(.)) / nrow(crime.join)))
...
ggplotly(
  ggplot(crime.red, aes(x = V3013, fill = V4399)) + geom_histogram() + scale_fill_manual(values =c("goldenrod", "#2055da"))+ theme_igray() + labs(x = "Age", y = 'Count', title = "Distribution of Age Colored by Whether Reported to Police or Not")
)
...
model.red.int <-
  glm(
    V4399 ~ (
      V3013 + V3015 + V3017 + V3034 + V3044 + V3048 + V3054 + V3083 + V3077 + V3081 + PERINTVNUM
    ) ^ 2,
    data = crime.red2,
    family = "binomial"
  )
...
stepAIC(model.step.int, trace = T, k = log(n))
...
crime.red2 <- within(crime.red2, intTerm <- relevel(intTerm, ref = 2))
data.frame(crime.red2$intTerm, crime.red2$V3034, crime.red2$V3044)

table(crime.red2$`34:44`)
#which(crime.red2$V3044 == "No" & crime.red2$V3034 == "Yes")

# Build a final int model, now just using the interaction term
model.fin <-
  glm(
    V4399 ~ V3013 + V3015 + V3017 + V3048 + V3054 + V3083 + V3077 + PERINTVNUM + intTerm +V3034:V3048 + V3034:V3081 + V3048:V3081,
    data = crime.red2,
    family = "binomial"
  )
#model.fin <- update(model.fin, . ~ . - V3034No:V3044Yes)
#model.fin <- update(model.fin, . ~ .  - V3034Yes:V3044No)
summary(model.fin)

model.fin <- glm(formula = V4399 ~ V3013 + V3015 + V3034 + V3044 + V3017 + V3048 + V3054 + 
    V3083 + V3077 + PERINTVNUM + V3034:V3044 + V3034:V3048 + 
    V3034:V3081 + V3048:V3081, family = "binomial", data = crime.red2)
summary(model.fin)

### Building interactions on full model (terrible idea I know)
#model.int.bad <- glm(V4399 ~ .^2, data = crime.red2, family = binomial())
#summary(model.int.bad)
# Epic fail 

model.step.int

# Experimenting with gam
# model.gam <- gam(V4399 ~ s(V3013,3) + V3015 + V3017 + V3048 + V3054 + V3083 + V3077 + s(PERINTVNUM,3) + V3034:V3044 + V3034:V3048 + V3034:V3081 + V3048:V3081, family = "binomial", data = crime.red2)
# summary(model.gam)

model.gam <- gam(V4399 ~ s(V3013,3) + V3015 + V3017 + V3034 + V3044+  V3048 + V3054 + V3083 + V3077 + PERINTVNUM + V3034:V3044 + V3034:V3048 + V3034:V3081 + V3048:V3081, family = "binomial", data = crime.red2)
summary(model.gam)
...
ggplotly(
  ggplot() + aes(x = model.fin$fitted.values, y = res.D.int) + geom_point() + stat_spline(spar = 1.1, size = 0.8, color = "firebrick") + labs(x = "Fitted Values", y = "Deviance Residuals", title = "Deviances Residuals vs Fitted Values for Model with Interactions")
)
...
ggplotly(
  ggplot() + aes(x = model.gam$fitted.values, y = res.D.gam) + geom_point() + stat_spline(spar = 1.2, size = 0.8, color = "firebrick") + labs(x = "Fitted Values", y = "Deviance Residuals", title = "Deviances Residuals vs Fitted Values for Model with Interactions")
)

runs.test(y = res.D.gam, plot.it = T)

# outliers
plot(model.step.int, which = 4)
plot(model.step.int, which = 5)
...
# 3. Assistance in interpreting residual plots, am I oversmoothing the spline? 
...
# 1. I have to completely rerun all the analysis, and EDA, with the new data merge, should not be too jealous, including type of crime da38090.0004$V4529 would be really interesting, lots of levels
